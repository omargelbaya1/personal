from tkinter import *
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 0.05
SHORT_BREAK_MIN = 0.1
LONG_BREAK_MIN = 0.2
reps=0
texts = ""
timer=None

# ---------------------------- TIMER RESET ------------------------------- # 
def reset_timer():
    global texts
    global reps

    window.after_cancel(timer)
    texts=""
    reps = 0
    canvas.itemconfig(timer_text,"00:00")
    checkmarks_label.config(text=texts)
    timer_label.config("Timer")



# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_timer():
    global reps
    work_secs=WORK_MIN*60
    short_break_secs=SHORT_BREAK_MIN*60
    long_break_secs=LONG_BREAK_MIN*60

    if reps % 2 ==0 and reps < 7:
        count_down(work_secs)
        reps+=1
        timer_label.config(text="Work", fg=GREEN, font=(FONT_NAME, 50, "bold"), bg=YELLOW)
    elif reps %2 ==1 and reps < 7:
        count_down(short_break_secs)
        reps += 1
        timer_label.config(text="Break", fg=PINK, font=(FONT_NAME, 50, "bold"), bg=YELLOW)

    else:
        count_down(long_break_secs)
        reps=0
        timer_label.config(text="Long Break", fg=RED, font=(FONT_NAME, 50, "bold"), bg=YELLOW)






# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):
    global reps
    global texts
    global timer
    count_min = math.floor(count/60)
    count_sec = count % 60
    if count_sec < 10:
        count_sec=f"0{count_sec}"

    canvas.itemconfig(timer_text,text=f"{count_min}:{count_sec}")
    if count >0:
        timer=window.after(1000,count_down,count-1)
    else:
        start_timer()
        if reps%2==0:
            texts+="✔"
            checkmarks_label.config(text=texts)



# ---------------------------- UI SETUP ------------------------------- #

#window
window =Tk()
window.title("Pomodoro")
window.config(padx=100,pady=50 ,bg=YELLOW)

#canvas
canvas = Canvas(width=200,height=224,bg=YELLOW,highlightthickness=0)
tomato_img=PhotoImage(file="tomato.png")
canvas.create_image(100,112,image=tomato_img)
timer_text=canvas.create_text(100,130,text="00:00",fill="white",font=(FONT_NAME,35,"bold"))
canvas.grid(column=1,row=1)

#Buttons
start_button = Button(text="Start",highlightbackground=YELLOW,command=start_timer)
start_button.grid(column=0,row=2)
reset_button = Button(text="Reset",highlightbackground=YELLOW,command=reset_timer)
reset_button.grid(column=2,row=2)


#Labels
timer_label=Label(text="Timer",fg=GREEN,font=(FONT_NAME,50,"bold"),bg=YELLOW)
timer_label.grid(column=1,row=0)

checkmarks_label=Label(fg=GREEN,font=(FONT_NAME,24,"bold"),bg=YELLOW)
checkmarks_label.grid(column=1,row=3)







window.mainloop()